...
static void add_rectangle(RenderQueue* rq, const Vec3 pos[3], Vec4 color, Vec3 normal) {
	...
	a.vertex[0] = { pos[0], normal, color };
	a.vertex[1] = { pos[1], normal, color };
	a.vertex[2] = { pos[3], normal, color };
	
	b.vertex[0] = { pos[3], normal, color };
	b.vertex[1] = { pos[1], normal, color };
	b.vertex[2] = { pos[2], normal, color };
...
static void render(RenderQueue* rq) {
	...
	glVertexPointer(3, GL_FLOAT, sizeof (Vertex), &rq->triangle->vertex[0].pos.x);
	glColorPointer(4, GL_FLOAT, sizeof (Vertex), &rq->triangle->vertex[0].color.x);
	glNormalPointer(GL_FLOAT, sizeof (Vertex), &rq->triangle->vertex[0].normal.x);
    
	glDrawArrays(GL_TRIANGLES, 0, 3 * rq->amount);
	rq->amount = 0;
}
